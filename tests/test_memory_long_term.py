"""Tests for memQrag.memory.long_term (SQLite long-term memory schema, Phase 4 PR 2).
...
def test_record_long_term_memory_returns_a_new_row_id(conn):
    long_term_memory_id = record_long_term_memory(conn, "What is the return policy?", [1, 2])
...
def test_new_record_starts_with_default_counters(conn):
    long_term_memory_id = record_long_term_memory(conn, "What is the return policy?", [1])
...
def test_update_long_term_memory_sets_success_count(conn):
    long_term_memory_id = record_long_term_memory(conn, "query", [1])
...
def test_get_all_long_term_memory_returns_most_recently_used_first(conn):
    older_id = record_long_term_memory(conn, "older query", [1])
    newer_id = record_long_term_memory(conn, "newer query", [2])
...
def test_best_document_ids_round_trips_empty_list(conn):
    long_term_memory_id = record_long_term_memory(conn, "no matches found", [])
